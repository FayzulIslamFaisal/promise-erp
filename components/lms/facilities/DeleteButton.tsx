...
import { SingleFacilityResponse } from "@/apiServices/facilitiesService";
...
const DeleteButton = ({ id }: DeleteButtonProps) => {
  ...
  const handleDelete = () => {
    startTransition(async () => {
      try {
        const res: SingleFacilityResponse = await DeleteFacilityAction(id);
        if (res.success) {
        ...
        console.error("Facility delete failed:", error);
        if (error instanceof Error) {
          toast.error(error.message);
        } else {
          toast.error("An unknown error occurred while deleting facility");
        }
      }
    });
  };
