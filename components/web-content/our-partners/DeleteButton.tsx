...
import { deletePartner, SinglePartnerResponse } from "@/apiServices/homePageAdminService";
...
const DeleteButton = ({ id }: DeleteButtonProps) => {
  ...
  const router = useRouter();
    ...
    startTransition(async () => {
      try { 
        const res: SinglePartnerResponse = await deletePartner(id);
        if (res.success) {
        ...
        if (error instanceof Error) {
          ...
          toast.error("An unknown error occurred while deleting partner");
        }
      } 
    });
  };
    ...
    <AlertDialog>
      <AlertDialogTrigger asChild>
        ...
        <AlertDialogHeader>
          ...
          <AlertDialogDescription> 
            This action cannot be undone. The partner will be permanently deleted.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
