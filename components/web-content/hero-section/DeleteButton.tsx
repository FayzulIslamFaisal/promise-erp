...
import DeleteHeroSectionAction from "@/actions/DeleteHeroSection";
import { SingleHeroSectionResponse } from "@/apiServices/homePageAdminService";
...
const DeleteButton = ({ id }: DeleteButtonProps) => {
  ...
  const handleDelete = () => {
    startTransition(async () => {
      try {
        const res: SingleHeroSectionResponse = await DeleteHeroSectionAction(id);
        if (res.success) {
          toast.success(res.message || "Hero section deleted successfully");
          router.refresh();
        } else {
